...
export function buildRecentOrderCardHTML(props) {
    ...
    // Truncate per line-clamp
    const hasMore = ingredientsText.length > 60;
    const showFullList = isExpanded && hasMore;

    return `
        <div class="bg-white dark:bg-card-dark rounded-2xl p-5 border border-slate-200 dark:border-slate-800/80 shadow-sm relative min-h-[120px]" data-order-id="${id}">
            <!-- Favorite Star (assoluto top-right) -->
            <button 
                class="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 ${is_favorite ? 'text-yellow-500' : 'text-slate-400'} border border-slate-200 dark:border-slate-700/50 z-10 hover:scale-110 transition-transform focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            ...
            >
                <span 
                    ...
                    aria-hidden="true"
                    ${is_favorite ? 'style="font-variation-settings: \'FILL\' 1, \'wght\' 400, \'GRAD\' 0, \'opsz\' 24;"' : ''}
                >star</span>
            </button>
            
            <div class="flex gap-4 ${showFullList ? 'mb-4' : ''}">
                <!-- Image -->
                <div class="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-slate-100 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                ...
                <div class="flex-grow pt-1 pr-8">
                    ${!showFullList ? `
                        <p class="text-[11px] font-medium text-slate-600 dark:text-slate-300 leading-relaxed mb-2 line-clamp-2">
            ...
            ${showFullList ? `
                <div class="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/50">
                    <button 
